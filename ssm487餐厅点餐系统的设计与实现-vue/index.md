# ssm487餐厅点餐系统的设计与实现+vue


# [项目清单 包安装运行](http://chenqi1990.site) 官网地址 http://chenqi1990.site

# [ssm487餐厅点餐系统的设计与实现+vue](https://github.com/GraduationProject-springboot/)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=85)
...
# 3 系统分析
对于餐厅点餐系统开发设计到的流程有，分析系统的功能，设计系统的结构，设计数据库，编码以及测试，其中，在系统分析中，所做的工作包括功能的确定，性能的分析等。
## 3.1 可行性研究
餐厅点餐系统开发实现分析需要从不同的角度来进行分析可行性，比如从时间角度，经济角度，甚至操作角度。从不同的角度分析可行性会让餐厅点餐系统开发具体化，进而达到辩证开发的正确性。
### 3.1.1 经济可行性
从经济方面分析是第一要素，没有经济的支持，任何项目都如水中捞月，无法实现。实现餐厅点餐系统，开发过程不需要额外的经济条件，用本人现有的计算机就可以实现，这方面不需要额外的支出。
### 3.1.2 时间可行性
餐厅点餐系统设计主要作为毕业设计，在题目确定之后，答辩之前使用的项目，对不同的开发进度上面都有时间的要求，总不至于答辩完成后才能实现功能，这个肯定不行，所以从时间上来分析项目的工作量，发现是可行的，符合正常开发时间。
### 3.1.3 操作可行性
...
![](/images/0400ssm/ssm487/blog.001.png)
...
![](/images/0400ssm/ssm487/blog.002.png)

图3.2 注册流程图

当需要用户登录的时候，肯定是要验证的，只有验证通过的用户才可以进行下一步操作，用户登录成功代表着用户模块的功能对登录用户进行了开放。流程就是如下面的图所示。

![](/images/0400ssm/ssm487/blog.003.png)

图3.3 登录流程图
## 3.4 系统功能分析
在对设计的总体要求理解了之后，就要把要求给具体化，也就是功能化，要尽量的把每个功能模块和模块之前的关系理清楚，必须符合正常人的行为逻辑才可以，并且尽量研究同类型的项目，这样能避免走弯路，最终才能得到设计的具体功能。

餐厅点餐系统把操作该系统的用户群分为两类，即管理员，用户。

管理员对于餐厅点餐系统操作的功能包括管理菜品，管理菜品订单，管理新闻，管理菜品留言，管理用户等。其用例图如图3.4所示：

![](/images/0400ssm/ssm487/blog.004.png)

图3.4 管理员用例图

用户对于餐厅点餐系统操作的功能包括购买菜品，查看新闻资讯，管理购物车，管理购买订单。其用例图如图3.5所示：

![](/images/0400ssm/ssm487/blog.005.png)
...
## 4.2 功能模块设计
对管理员具体功能的设计结果将以图4.1所示的管理员功能结构图来进行体现。管理员对于餐厅点餐系统操作的功能包括管理菜品，管理菜品订单，管理新闻，管理菜品留言，管理用户等。

![](/images/0400ssm/ssm487/blog.006.png)

图4.1 管理员功能结构图

对用户具体功能的设计结果将以图4.2所示的用户功能结构图来进行体现。用户对于餐厅点餐系统操作的功能包括购买菜品，查看新闻资讯，管理购物车，管理购买订单。

![](/images/0400ssm/ssm487/blog.007.png)
...
（1）菜品订单包括的属性有取餐码，就餐类型，支付类型等。其属性图如下。

![](/images/0400ssm/ssm487/blog.008.png)

图4.4 菜品订单实体属性图

（2）用户包括的属性有密码，姓名，性别等。其属性图如下。

![](/images/0400ssm/ssm487/blog.009.png)

图4.5 用户实体属性图

（3）菜品包括的属性有菜品库存，现价，菜品名称等。其属性图如下。

![](/images/0400ssm/ssm487/blog.010.png)

图4.6 菜品实体属性图

（4）菜品留言包括的属性有留言内容，留言时间等。其属性图如下。

![](/images/0400ssm/ssm487/blog.011.png)

图4.7 菜品留言实体属性图

（5）设计的各实体间关系E-R图如下。

![](/images/0400ssm/ssm487/blog.012.png)
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(11)|否|
|goods\_name|菜品名称 |varchar(200)|是|
|goods\_types|菜品类型 |int(11)|是|
|goods\_photo|菜品照片|varchar(200)|是|
|goods\_kucun\_number|菜品库存|int(11)|是|
|goods\_old\_money|菜品原价|decimal(10,2)|是|
|goods\_new\_money|现价|decimal(10,2)|是|
|goods\_price|购买获得积分|int(11)|是|
|shangxia\_types|是否上架|int(11)|是|
|goods\_delete|逻辑删除|int(11)|是|
|goods\_content|菜品简介|text|是|
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(11)|否|
|goods\_order\_uuid\_number|取餐码|varchar(200)|是|
|goods\_id|菜品|int(11)|是|
|yonghu\_id|用户|int(11)|是|
|buy\_number|购买的数量|int(200)|是|
|order\_types|就餐类型|int(11)|是|
...
## 5.1 管理员功能实现
### 5.1.1 菜品信息管理
该功能主要用于实现对菜品基本信息的管理，菜品信息管理界面的运行效果见图5.1。在此界面，管理员根据菜品名称查询菜品，同时能够对菜品进行库存管理，包括库存增加，库存减少等操作，还可以对菜品下架，更改菜品信息等。

![](/images/0400ssm/ssm487/blog.013.png)

图5.1 菜品信息管理界面
### 5.1.2 菜品订单管理
该功能主要用于实现对菜品订单基本信息的管理，菜品订单管理界面的运行效果见图5.2。在此界面，管理员对菜品订单进行出餐，根据用户姓名来查询菜品订单，对菜品订单的明细信息进行查看，删除菜品订单等。

![](/images/0400ssm/ssm487/blog.014.png)

图5.2 菜品订单管理界面
### 5.1.3 用户管理
该功能主要用于实现对用户基本信息的管理，用户管理界面的运行效果见图5.3。在此界面，管理员删除用户，查询用户，新增用户等。

![](/images/0400ssm/ssm487/blog.015.png)

图5.3 用户管理界面
### 5.1.4 新闻信息管理
该功能主要用于实现对新闻基本信息的管理，新闻信息管理界面的运行效果见图5.4。在此界面，管理员在此界面负责发布新闻，对新闻的基本信息进行更改，查询，删除。

![](/images/0400ssm/ssm487/blog.016.png)

图5.4 新闻信息管理界面
### 5.1.5 菜品留言管理
该功能主要用于实现对菜品留言基本信息的管理，菜品留言管理界面的运行效果见图5.5。在此界面，管理员需要回复用户对菜品发布的留言信息，可以查询菜品留言，删除菜品留言等操作。

![](/images/0400ssm/ssm487/blog.017.png)

图5.5 菜品留言管理界面
## 5.2 用户功能实现
### 5.2.1 菜品信息
菜品信息界面的运行效果见图5.6。在此界面，用户查看本界面的菜品，对菜品的相关介绍进行了解，可以发布对该菜品的相关留言，同时可以参与该菜品的购买操作。

![](/images/0400ssm/ssm487/blog.018.png)

图5.6 菜品信息界面
### 5.2.2 购物车
购物车界面的运行效果见图5.7。在此界面，用户负责对购物车的菜品信息进行管理，其中包括数量的修改，对不需要购买的菜品信息进行删除，对需要购买的菜品进行结算。

![](/images/0400ssm/ssm487/blog.019.png)

图5.7 购物车界面
### 5.2.3 确认下单
确认下单界面的运行效果见图5.8。在此界面，用户对购买清单进行检查，然后进行订单提交并支付。

![](/images/0400ssm/ssm487/blog.020.png)

图5.8 确认下单界面
### 5.2.4 我的菜品订单
我的菜品订单界面的运行效果见图5.9。在此界面，用户对已经支付的菜品订单信息进行检查，发现误购的菜品可以及时申请退款。

![](/images/0400ssm/ssm487/blog.021.png)
