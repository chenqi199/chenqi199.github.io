...
# [0162springboot美食推荐商城的设计与实现](https://github.com/GraduationProject-springboot/0162springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1jqaLe1ECs&bvid=BV1jqaLe1ECs&cid=500001610829887&p=60)
...
[]{#refheading___toc9299 .anchor}

[目 录 III](#refheading___toc9299)
...
[4.3 数据库设计 13](#__RefHeading___Toc5163)

[4.3.1 数据库E-R图设计 13](#__RefHeading___Toc26636)
...
[6 系统测试 20](#__RefHeading___Toc3226)
...
[6.2.2 添加班级功能测试 21](#添加班级功能测试)

[6.3 测试结果分析 21](#测试结果分析)

[结 论 23](#结-论)

[致 谢 24](#__RefHeading___Toc11885)
...
表4.5美食表
  ...
  ------ --------------------- -------------------- ------------------- --------
   序号          列名                数据类型              说明          允许空
    ...
    4        meishi_photo             String             美食照片          是

    5        meishi_types            Integer             美食类型          是

    6     meishi_kucun_number        Integer             美食库存          是

    7        meishi_price            Integer           购买获得积分        是

    8      meishi_old_money         BigDecimal           美食原价          是

    9      meishi_new_money         BigDecimal             现价            是

    10      meishi_clicknum          Integer             点击次数          是

    11      shangxia_types           Integer             是否上架          是

    12       meishi_delete           Integer             逻辑删除          是
...
表4.8美食订单表
  ...
  ------ ---------------------------- -------------------- ------------------- --------
   序号              列名                   数据类型              说明          允许空
    ...
    2      meishi_order_uuid_number          String              订单号           是
    ...
    6      meishi_order_true_price         BigDecimal           实付价格          是

    7         meishi_order_types            Integer             订单类型          是

    8     meishi_order_payment_types        Integer             支付类型          是
...
表4.9商家表
  ...
  ------ ----------------------- -------------------- ------------------- --------
   序号           列名                 数据类型              说明          允许空
    ...
    6     shangjia_xingji_types        Integer           商家星级类型        是
...
![](media/image17.png){width="5.7625in" height="2.908333333333333in"}
...
美食推荐商城需要管理员添加班级信息，班级名称和班级的说明都是程序设置的必填数据，而且添加的班级名称也不能够是数据库里面的存在的数据。对添加班级功能测试时，测试数据在下表展示。
...
第一点，可以确定开发出来的美食推荐商城是符合同类型系统的要求；

第二点，用户要求美食推荐商城需要具备的功能都已开发完成并能够正常使用；

第三点，美食推荐商城界面简洁美观，操作流程清晰明了；

第四点，美食推荐商城的安全性，以及运行性能符合开发要求。

至此，美食推荐商城可以确保所有功能能够按照预期目标进行运行，可以正式投入生活中运行使用！

# 结 论

通过对美食推荐商城的开发，让我深刻明白开发一个程序软件需要经历的流程，当确定要开发一个美食推荐商城的程序时，我在开发期间，对其功能进行合理的需求分析，然后才是程序软件的功能的框架设计，数据库的实体与数据表设计，程序软件的功能详细界面实现，以及程序的功能测试等进行全方位的细致考虑，虽然在此过程中，各个环节都遇到了大大小小的困难，但是通过对这些问题进行反复的分析，深入的思考，借助各种相关文献资料提供的方法与解决思路成功解决面临的各个问题，最后成功的让我开发的美食推荐商城得以正常运行。

美食推荐商城在功能上面是基本可以满足用户对系统的操作，但是这个程序软件也有许多方面是不足的，因此，在下一个时间阶段，有几点需要改进的地方需要提出来，它们分别是：

（1）操作页面可以满足用户简易操作的要求，但是在页面多样化设计层面上需要把一些比较丰富的设计结构考虑进来。

（2）程序软件的总体安全性能需要优化，例如程序的退出安全性，以及程序的并发性等问题都需要进行安全性升级，让开发的美食推荐商城与现实中的相关网站更贴合。

（3）需要对程序的数据结构方面，程序的代码方面等进行优化，让运行起来的程序可以保持稳定运行，也让程序能够保证短时间内处理相关事务，节省处理事务的时间，提高事务处理的效率，同时对服务器上资源占用的比例进行降低。

美食推荐商城的开发一方面是对自身专业知识技能进行最终考核，另一方面也是让自己学会独立解决程序开发过程中所遇到的问题，掌握将理论知识运用于程序开发实践的方法。美食推荐商城的开发最终目标就是让系统更具人性化，同时在逻辑设计上，让系统能够更加的严谨。

[]{#__RefHeading___Toc11885
...
1.  李德华.基于SSM技术的美食推荐商城的研究与实现\[J\].信息与电脑(理论版),2019,31(17):51-53.

    \[2\]屈敬华,王晓孟.美食推荐商城的设计与实现\[J\].计算机时代,2019(01):43-45.
...
### 0162springboot美食推荐商城的设计与实现 项目图片
![图片](/images/0162springbootimg_014.jpg)
![图片](/images/0162springbootimg_015.jpg)
![图片](/images/0162springbootimg_001.jpg)
![图片](/images/0162springbootimg_003.jpg)
![图片](/images/0162springbootimg_002.jpg)
![图片](/images/0162springbootimg_016.jpg)
![图片](/images/0162springbootimg_012.jpg)
![图片](/images/0162springbootimg_006.jpg)
![图片](/images/0162springbootimg_007.jpg)
![图片](/images/0162springbootimg_013.jpg)
![图片](/images/0162springbootimg_005.jpg)
![图片](/images/0162springbootimg_011.jpg)
![图片](/images/0162springbootimg_010.jpg)
![图片](/images/0162springbootimg_004.jpg)
![图片](/images/0162springbootimg_009.jpg)
![图片](/images/0162springbootimg_008.jpg)
