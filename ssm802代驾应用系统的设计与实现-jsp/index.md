...
# [ssm802代驾应用系统的设计与实现+jsp](https://github.com/GraduationProject-springboot/)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1xRe2eHEsa?p=3)
...
## 1.2 选题意义
当开发软件变成了一个潮流之后，会发现不管任何行业都能开发适合自己的软件，不管是内部员工管理，还是财务管理，甚至业务管理都可以数据化，并且可以对这些数据集中处理，进而根据数据处理结果就自然而然的提高了管理水平，最重要的是，开发一个软件投入使用，开发过程其实就是梳理行业痛点的过程，就是让软件来弥补行业的管理漏洞，不断的优化事物的处理流程。代驾应用系统就是一款专门开发的软件，通过Web技术，让使用者可以在任何一台智能设备上面通过浏览器进行操作使用，对数据的处理不在局限于地域距离，只要软件开发到位，使用起来方便，达到预期目的，会发现有软件和没有软件的区别是很大的，有了这款软件之后，会发现数据的存储安全方面，比起之前的满屋寻找记录的优势是多么的巨大。
## 1.3 研究内容
...
### 3.1.2市场可行性分析
代驾应用系统是根据用户经常使用的页面操作流程来进行设计的，并且页面保证统一，是根据市场的反应来调研开发的，所以市场可行性分析是必须的。
### 3.1.3法律可行性分析
在本次开发过程中，用到的不管是eclise还是Java还是MySQL都是开源免费的，不存在技术上的法律问题，并且是专门开发的软件，不是抄袭别人的，不存在法律上的风险。所以，从法律可行性分析，是可以的。
## 3.2系统流程分析
...
![](/images/0800ssm/ssm802/blog.025.png) 
