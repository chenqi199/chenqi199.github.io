...
# [0188springboot基于SpringBoot电商平台的设计与实现](https://github.com/GraduationProject-springboot/0188springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1jqaLe1ECs&bvid=BV1jqaLe1ECs&cid=500001610833798&p=86)
...
电商平台可以实现商家管理，商品订单管理，用户管理，商品管理，商品评价管理等功能。该系统采用了Mysql数据库，Java语言，Spring
Boot框架等技术进行编程实现。
...
old technologies. Because the traditional commodity transaction
information management is difficult, the fault tolerance rate is low,
and the management personnel are labor-intensive and time-consuming to
process the data, an e-commerce platform has been specially developed to
solve this problem, which can solve many problems.

The e-commerce platform can realize the functions of merchant
management, commodity order management, user management, commodity
management, commodity evaluation management and so on. The system uses
Mysql database, Java language, Spring Boot framework and other
technologies for programming.

The e-commerce platform can improve the efficiency of solving the
problem of commodity transaction information management, optimize the
processing process of commodity transaction information, and ensure the
security of commodity transaction information data. It is a very
...
[第5章 系统实现 23](#__RefHeading___Toc31437)

[5.1 管理员功能实现 23](#管理员功能实现)

[5.1.1 公告信息管理 23](#公告信息管理)

[5.1.2 用户管理 23](#用户管理)

[5.1.3 商家管理 24](#商家管理)

[5.1.4 商家星级管理 24](#商家星级管理)
...
[第6章 系统测试 29](#__RefHeading___Toc9413)
...
本课题研究和开发电商平台，让安装在计算机上的该系统变成管理人员的小帮手，提高商品交易信息处理速度，规范商品交易信息处理流程，让管理人员的产出效益更高。

## 1.2 课题意义

传统处理数据，必须是一张张纸，然后处理完毕又是统计在一张张纸上面，不断的重复处理，最终有个结果给最高层作为参考，这个模式在互联网没有出现之前，是一种常见的事情，信息管理的效率提不上去，人多不一定力量大，因为人多肯定更加消耗资源，并且因为人类需要休息，需要管理，思想会不统一，会偷懒，所以人们研究出专门帮助人们计算的机器，就是计算机的前身，到了互联网时代，人们发现完全可以让程序供应商提供解决方案，自己挑选自己合适的方案来提高自己的产出比。所以在日常工作和生活中会发现各种各样方便人们的工具。

本课题研发的电商平台，就是提供商品交易信息处理的解决方案，它可以短时间处理完信息，并且这些信息都有专门的存储设备，而且数据的备份和迁移都可以设定为无人值守，从人力角度和信息处理角度以及信息安全角度，电商平台是完胜传统纸质操作的。

## 1.3 研究内容

本文对电商平台的设计与实现分成六个章节来说明。

第1章：研究电商平台的背景，以及开发电商平台的意义。

第2章：对开发电商平台的环境还有技术进行说明。

第3章：分析电商平台的可行性，性能，流程以及功能。

第4章：设计电商平台的功能结构，设计数据库E-R图以及对数据表的存储结构进行设计。

第5章：实现电商平台的功能并进行功能界面展示。

第6章：对系统测试进行阐述，以及对本系统部分功能进行检测。

# 第2章 开发环境与技术

本章节对开发电商平台需要搭建的开发环境，还有电商平台开发中使用的编程技术等进行阐述。
...
在技术实现层次，分析了好几种技术实现方法，并且都有对应的成功案例，也有很多开源模块可以进行参考，所以从技术可行性分析来讲，实现电商平台是没有问题的。

### 3.1.2 经济可行性

对于身为学生的开发者而言，在经济资源上面可用者很少，为了开发电商平台，通过开发软件对硬件的要求，发现自己的电脑是完全能用来开发的，并且学校机房的配置也可以达到要求。最重要的是选择的技术都可以在网上找到不花钱的教程以及资料，因为不花钱，所以经济方面是具有可行性的。

### 3.1.3 操作可行性

电商平台的具体实现，本身参考人类的正常操作逻辑，把常用的操作习惯当做主要的导航实现，可以让使用者更快速的理解并且上手操作，实现符合逻辑的操作流程是操作可行性的具体体现。

以上就是从不同的角度来分析，确保了电商平台的正常开展。

## 3.2 系统流程

电商平台投入使用后，使用者如果能看到相应的流程操作图会提高程序的理解能力。

### 3.2.1 操作流程

使用者在操作电商平台中，应该按照本系统提供的操作流程（图3.1即为本系统的操作流程图）进行操作，可以减少操作失误，从而节省进入电商平台的时间。
...
电商平台通过登录功能（图3.2即为其登录的流程）引导使用者进入指定的功能操作区，也避免非本系统的用户享受本系统提供的服务以及查看本系统提供的信息，进而保证用户安全。
...
电商平台在经过长期使用后，会产生很多的数据信息。为了腾出存储空间存放更多的数据，本系统数据库中存储的数据，一些没有参考价值的数据需要进行删除（图3.3即为删除信息的流程），删除数据过程中，为避免误删，使用者要根据系统的提示来决定是否删除数据。
...
[]{#__RefHeading___Toc31437 .anchor}
...
图5.1
即为编码实现的公告信息管理界面，管理员在公告信息管理界面中可以对界面中显示的所有公告信息进行更改，查询，删除。
...
图5.3
即为编码实现的商家管理界面，管理员在商家管理界面中新增商家，更改商家的营业执照，商家星级信息等，可以删除需要删除的商家信息。
...
图5.4
即为编码实现的商家星级管理界面，管理员在商家星级管理界面修改商家星级信息，查询商家星级信息，删除商家星级信息等。
...
图5.5
即为编码实现的商品管理界面，商家在商品管理界面中为商品的库存进行增加，减少操作，可以上架商品，下架商品以及更改商品信息等。
...
图5.6
即为编码实现的商品评价管理界面，商家在商品评价管理界面中查看用户对商品的评价信息，商家需要对商品评价进行回复。
...
图5.7
即为编码实现的商品订单管理界面，商家在商品订单管理界面中参与商品订单发货，商品订单查询，商品订单删除等操作。
...
图5.11
即为编码实现的商品订单界面，用户在商品订单界面中可以退款，可以评价订单以及查看订单详情等。
...
[]{#__RefHeading___Toc9413 .anchor}**6章**
...
[]{#__RefHeading___Toc2183 .anchor}**致 谢**
...
### 0188springboot基于SpringBoot电商平台的设计与实现 项目图片
![图片](/images/0188springbootimg_001.jpg)
![图片](/images/0188springbootimg_003.jpg)
![图片](/images/0188springbootimg_002.jpg)
![图片](/images/0188springbootimg_006.jpg)
![图片](/images/0188springbootimg_007.jpg)
![图片](/images/0188springbootimg_005.jpg)
![图片](/images/0188springbootimg_004.jpg)
