...
# [0283springboot问卷调查系统的设计与实现](https://github.com/GraduationProject-springboot/0283springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1jqaLe1EbH&bvid=BV1jqaLe1EbH&cid=500001610886330&p=49)
...
[4.3 数据库设计 10](#__RefHeading___Toc6592)

[4.3.1 数据库设计 10](#__RefHeading___Toc27013)

[4.3.2 数据库E-R 图 10](#__RefHeading___Toc3238)
...
[5.1.2 新闻资讯信息管理 13](#新闻资讯信息管理)

[5.1.3 新闻资讯类型管理 14](#新闻资讯类型管理)
...
[6.4.2 添加新闻资讯类别测试 16](#添加新闻资讯类别测试)

[6.5测试结果 16](#测试结果)

[结 论 17](#结-论)

[参考文献 19](#__RefHeading___Toc27077)
...
B/S结构实际上就是Browser/Server（浏览器/服务器）的缩写，B/S结构是目前软件开发中的主流结构，大多数软件开发者都采用B/S这样的三层体系结构。采用B/S结构开发的问卷调查系统能够大大方便用户使用操作，用户随时随地都可以进行问卷调查系统的访问操作，只需要用户拥有一台会连上网络的计算机就行了，并且使用B/S
模式开发的问卷调查系统是基于浏览器的操作方式，在与用户进行交互以及程序的表现方式上面将会更加吸引人，这样的模式也给程序员开发程序降低了难度，程序开发成本也会相应缩减不少。我们在使用问卷调查系统期间，系统也会随着用户需求进行完善升级，使用B/S
模式开发的问卷调查系统就完全不需要担心，因为它的升级速度快，升级产生的成本费用很低，大多依赖于用户自己在网络上下载最新版本进行安装完成程序更新。下图为B/S结构图。
...
AB公司在很早之前就收购过一个关系型数据库，它是现在的Mysql数据库。所以问卷调查系统后台数据库使用的是MySQL进行数据库方面的开发工作的，MySQL它是微软开发的一款平台软件，这个软件可以给用户提供高效率的智能数据，并且数据信息还是很可靠，使用它进行数据存储可以满足大众企业管理各种各样的数据信息的需求。MySQL在MySQL版本里面它是最全面的，也是最强大的开发平台，
...
![](media/image5.jpeg){width="6.1in" height="2.6534722222222222in"}\
图4.2 问卷表实体属性图
...
![](media/image7.jpeg){width="6.1in" height="2.6534722222222222in"}\
图4.4 答题详情表实体属性图

（5）下图是字典表实体和其具备的属性。

![](media/image8.jpeg){width="6.096527777777778in"
...
![](media/image11.jpeg){width="5.833333333333333in" height="2.4in"}\
图4.8 用户表实体属性图
...
表4.2问卷表表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    3       exampaper_date          Integer            时长(分钟)         是

    4     exampaper_jieshuyu         String              结束语           是
...
表4.3试题表表
  ...
  ------ ----------------------- -------------------- -------------------------- --------
   序号           列名                 数据类型                  说明             允许空
    ...
    5      examquestion_types          Integer                 试题类型             是

    6     examquestion_sequence        Integer         试题排序，值越大排越前面     是
...
表4.4问卷调查记录表表
  ...
  ------ ------------------------ -------------------- -------------------- --------
   序号            列名                 数据类型               说明          允许空
    ...
    2     examrecord_uuid_number         String            问卷调查编号        是

    3           yonghu_id               Integer            问卷调查用户        是

    4          exampaper_id             Integer         所属问卷id（外键）     是

    5          insert_time                Date             问卷调查时间        是
...
表4.5答题详情表表
  ...
  ------ --------------------------- -------------------- ------------------- --------
   序号             列名                   数据类型              说明          允许空
    ...
    5      examredetails_myanswer           String             用户选项          是
...
![](media/image12.png){width="6.090277777777778in"
...
![](media/image13.png){width="6.0881944444444445in"
...
问卷调查系统的测试选用的测试平台是IDEA平台环境，测试时首先需要用户打开MySQL数据库进行数据库文件的附加操作，然后打开IDEA，选择文件打开网站，把问卷调查系统的程序添加进入IDEA平台中，接着把文件部署到tomcat服务器里面，最后运行程序，这时用户可以操作系统里面的各个功能，看看程序有没有达到用户的要求。
...
管理员可以在添加新闻资讯类别界面输入新闻资讯类别名称信息，如果数据信息为空，系统会给出相应提示。只有所有的数据信息都合理输入，管理员才能完成新闻资讯类别的添加操作。测试数据见下表：
...
表6.2添加新闻资讯类别测试表
  ...
  添加新闻资讯类别   管理员登陆后点击添加新闻资讯类别按钮，添加新闻资讯类别信息   类别名称为空       添加失败，弹出提示按钮   添加失败，提示请填写内容
...
经过此次对问卷调查系统的综合性测试，我们不难发现程序的功能并没有出现明显的逻辑性错误，用户在进行功能操作时，程序基本能根据用户操作情况给出相应的反馈。程序质量以及可靠性在系统的反复测试中都经过了严格检验，程序投入生活使用完全没有问题。

# 结 论

问卷调查系统的开发制作，从题目确定到成品完成，自己投入的精力与心血是非常多的。从问卷调查系统的前台页面实现，到问卷调查系统的后台代码的编辑，我用到的软件包括了数据库软件Mysql，Java开发工具IDEA，办公软件Office，浏览器软件Fireworks，图像处理软件Photoshop等，这也是我第一次使用Java语言，开发的这个比较简单的问卷调查系统。

开发问卷调查系统让我明白知识理论与现实实践的差距还是挺大的，很多时候，我们在学习课程知识的时候，都感觉书本上的内容，老师一点就通，然而这次毕设就真正考验了我，原来理论知识和现实实践完全是两码事，开发问卷调查系统一方面让我对在校学习的知识起了一个巩固作用，另一方面让我明白独立学习知识的重要性。在毕设作品完成期间，我才知道要开发一款软件需要经历很多繁琐的步骤，从开始的选题到系统需求分析以及功能板块的设计，还有系统详细实现以及系统测试，每个环节看似简单，其实暗藏很多知识点，这些都需要自己去网上或者是学校的图书馆查阅相对应的资料。每个环节都需要认真对待，要是系统需求分析没有做好，那么在系统设计与实现过程中就会出现很多错误，要么功能不全，要么就是程序开发出来根本不符合用户要求，所以程序的开发是一个知识再造的过程，一个人独自开发完一款软件也就深知这其间的各种不易了。
...
[]{#__RefHeading___Toc27077 .anchor}
...
\[13\]刘小华,化琼珍.问卷调查系统的构建\[J\].物流技术与应用,2015(03):100-101.

\[14\]王锋.问卷调查系统构建的研究\[J\].商场现代化,2016(24):35-36.

\[15\]王育才.问卷调查系统构建的分析\[J\].中国管理信息化,2017,20(22):54-55.
...
### 0283springboot问卷调查系统的设计与实现 项目图片
![图片](/images/0283springbootimg_001.jpg)
![图片](/images/0283springbootimg_003.jpg)
![图片](/images/0283springbootimg_002.jpg)
![图片](/images/0283springbootimg_006.jpg)
![图片](/images/0283springbootimg_007.jpg)
![图片](/images/0283springbootimg_005.jpg)
![图片](/images/0283springbootimg_004.jpg)
