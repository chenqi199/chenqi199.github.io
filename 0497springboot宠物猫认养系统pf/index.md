...
# [0497springboot宠物猫认养系统pf](https://github.com/GraduationProject-springboot/0497springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1ULbQeREgz&bvid=BV1ULbQeREgz&cid=500001616882875&p=94)
...
本宠物领养系统采用基于JAVA语言实现、架构模式选择B/S架构，即JAVA启动运行的客户端与服务器的结构，基于
J2EE的基本标准，Tomcat7.0及以上作为运行服务器支持，基于、JAVA、springboot、vue等主要技术设计，idea作为开发环境，数据库采用MYSQL5.7以上。
...
Java主要采用CORBA技术和安全模型，可以在互联网应用的数据保护。它还提供了对EJB（Enterprise
JavaBeans）的全面支持，java servlet
API，springboot和XML技术。JAVA语言是一种面向对象的语言，它通过提供最基本的方法来完成指定的任务，开发者只需要知道一些概念就能够编写出一些应用程序。Java程序相对较小，其代码能够在小机器上运行。Java是一种计算机编程语言，具有封装、继承和多态性三个主要特性，广泛应用于企业Web应用程序开发和移动应用程序开发。
...
[]{#__RefHeading___Toc29901 .anchor}表4.1收货地址表
  ...
  ------ ----------------- -------------------- ------------------- --------
   序号        列名              数据类型              说明          允许空
    ...
    3      address_name          varchar              收货人           是

    4      address_phone         varchar               电话            是

    5      address_dizhi         varchar               地址            是
...
表4.2宠物表
  ...
  ------ ------------------- -------------------- ------------------- --------
   序号         列名               数据类型              说明          允许空
    ...
    2       chongwu_name           varchar             宠物名称          是

    3       chongwu_photo          varchar             宠物照片          是

    4       chongwu_types          Integer             宠物品种          是

    5     chongwuleix_types        Integer             宠物类型          是

    6        chongwu_age           Integer             宠物年龄          是

    7       xingbie_types          Integer             宠物性别          是

    8      chongwu_tezheng         varchar             宠物特征          是

    9       dongwu_types           Integer             宠物状态          是

    10     chongwu_delete          Integer             逻辑删除          是

    11     chongwu_content         varchar             宠物简介          是
...
表4.3宠物领养申请表
  ...
  ------ --------------------------- -------------------- ------------------- --------
   序号             列名                   数据类型              说明          允许空
    ...
    4        chongwu_yuyue_text            varchar             领养原因          是

    5        chongwu_yuyue_types           Integer             领养状态          是

    6     chongwu_yuyue_yesno_types        Integer             领养审核          是

    7     chongwu_yuyue_yesno_text         varchar             审核结果          是
...
表4.4宠物用品表
  ...
  ------ ----------------------------- -------------------- ------------------- --------
   序号              列名                    数据类型              说明          允许空
    ...
    2         chongwuyongpin_name            varchar           宠物用品名称        是

    3        chongwuyongpin_photo            varchar           宠物用品照片        是

    4        chongwuyongpin_types            Integer           宠物用品类型        是

    5     chongwuyongpin_kucun_number        Integer           宠物用品库存        是

    6      chongwuyongpin_old_money         BigDecimal             原价            是

    7      chongwuyongpin_new_money         BigDecimal             现价            是

    8       chongwuyongpin_clicknum          Integer             点击次数          是

    9           shangxia_types               Integer             是否上架          是

    10       chongwuyongpin_delete           Integer             逻辑删除          是

    11      chongwuyongpin_content           varchar           宠物用品简介        是
...
表4.5宠物用品收藏表
  ...
  ------ --------------------------------- -------------------- ------------------- --------
   序号                列名                      数据类型              说明          允许空
    ...
    2            chongwuyongpin_id               Integer             宠物用品          是

    3                yonghu_id                   Integer               用户            是

    4     chongwuyongpin_collection_types        Integer               类型            是
...
表4.6宠物用品订单表
  ...
  ------ ------------------------------------ -------------------- ------------------- --------
   序号                  列名                       数据类型              说明          允许空
    ...
    2      chongwuyongpin_order_uuid_number         varchar              订单号           是

    3                 address_id                    Integer             收货地址          是

    4             chongwuyongpin_id                 Integer             宠物用品          是
...
表4.7字典表表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    2        dic_code           varchar               字段            是

    3        dic_name           varchar              字段名           是

    4       code_index          Integer               编码            是

    5       index_name          varchar             编码名字          是
...
表4.8萌宠讨论表
  ...
  ------ ------------------- -------------------- ------------------- --------
   序号         列名               数据类型              说明          允许空
    ...
    2        forum_name            varchar             帖子标题          是

    3         yonghu_id            Integer               用户            是

    4         users_id             Integer              管理员           是

    5       forum_content          varchar             发布内容          是
...
表4.9留言反馈表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    3      liuyan_name          varchar             留言标题          是

    4      liuyan_text          varchar             留言内容          是

    5      insert_time            Date              留言时间          是

    6       reply_text          varchar             回复内容          是
...
表4.10通知公告表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    2       news_name           varchar             公告标题          是

    3       news_types          Integer             萌宠讨论          是

    4       news_photo          varchar             公告图片          是

    5      insert_time            Date              添加时间          是

    6      news_content         varchar             公告详情          是
...
表4.11用户表
  ...
  ------ ------------------ -------------------- ------------------- --------
   序号         列名              数据类型              说明          允许空
    ...
    2       yonghu_name           varchar             用户姓名          是

    3       yonghu_photo          varchar             用户头像          是

    4     yonghu_id_number        varchar             身份证号          是

    5       yonghu_phone          varchar             联系方式          是

    6       yonghu_email          varchar             电子邮箱          是
...
如图5.2显示的就是宠物用品管理页面，此页面提供给管理员的功能有：查看已发布的宠物用品数据，修改宠物用品，宠物用品作废，即可删除，还进行了对宠物用品名称的模糊查询
宠物用品信息的类型查询等等一些条件。
...
宠物猫认养系统的目标用户最终面向的是系统的功能，所以检验本系统的功能也是一项重要任务，只有该系统的功能都能保证正常使用，才会吸引更多的用户来使用。
...
通过测试，可以发现宠物猫认养系统具备完善的功能，同时具备易操作性和易维护性的特点，在系统安全性上也有比较健全的数据保护机制。至此，宠物猫认养系统可以投入使用。

# 结 论

此时项目已经完成，即使实施的时间不是很长，但是这个过程中需要准备很长的一段时间去对系统设计开发所实际到的技术进行学习。在学习的过程中，我逐渐认识得到了我自身存在的一些不足。对于一些控制是必要的应用技能，能够理解，整个过程中仅仅是一个掌握了常用的性能和控制方法，我觉得挺容易的。从该系统中，系统的分析和设计的调查数据，并且已经经历了几个月，并努力几个月，该系统已经完成。很显然，该系统仍有很多不成熟，在系统设计过程中有许多技术缺陷存在。在设计的过程中也涉及到了很多自己无法解决的问题，主要通过找专业的网站和论坛来解决这些问题，对于圆满完成我的毕业设计，他们也贡献了很大一部分力量。系统的开发环境和配置都是可以自行安装的，系统使用Java开发，使用比较成熟的Mysql数据库进行对系统前台及后台的数据交互，根据技术语言对数据库，结合需求进行修改维护，可以使得系统运行更具有稳定性和安全性，从而完成实现系统的开发。
...
1.  卢鸿,朱隆尹.宠物猫认养系统查询系统优化设计及应用\[J\].南昌大学学报(理科版),2016,36(04):402-404.

    \[2\]高岳,张腾.基于基站定位的宠物猫认养系统查询系统设计\[J\].无线互联科技,2015(03):32-35.

\[3\]钱佳加,唐海江,夏莎莎,姚宇杰,张祥杰.宠物猫认养系统查询系统的设计与实现\[J\].现代商业,2015(17):67-68.
...
### 0497springboot宠物猫认养系统pf 项目图片
![图片](/images/0497springbootimg_001.jpg)
![图片](/images/0497springbootimg_003.jpg)
![图片](/images/0497springbootimg_002.jpg)
![图片](/images/0497springbootimg_006.jpg)
![图片](/images/0497springbootimg_007.jpg)
![图片](/images/0497springbootimg_005.jpg)
![图片](/images/0497springbootimg_004.jpg)
![图片](/images/0497springbootimg_008.jpg)
