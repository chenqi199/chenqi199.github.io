...
![](media/image10.jpeg){width="2.9159722222222224in"
height="1.5270833333333333in"}\
图4.7 比赛首发名单实体属性图
...
表4.2教练表
  ...
  ------ ----------------- -------------------- ------------------- --------
   序号        列名              数据类型              说明          允许空
    ...
    3     jiaolian_phone          String              手机号           是

    4     jiaolian_email          String             电子邮箱          是

    5     jiaolian_delete        Integer               假删            是
...
表4.3加入的球队表
  ...
  ------ ---------------------- -------------------- ------------------- --------
   序号           列名                数据类型              说明          允许空
    ...
    3          yonghu_id              Integer               球员            是

    4     jiarudeqiudui_number       BigDecimal             工资            是
...
表4.5球队信息表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    3      qiudui_types         Integer             球队类型          是

    4     qiudui_address         String             所属地点          是

    5      jiaolian_id          Integer               教练            是

    6     qiudui_content         String             球队简介          是
...
表4.6比赛首发名单表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    3     shoufamingdan_file         String             名单下载          是
...
表4.7训练计划表
  ...
  ------ ---------------------- -------------------- ------------------- --------
   序号           列名                数据类型              说明          允许空
    ...
    3      xunlianjihua_name           String               标题            是

    4     xunlianjihua_tianshu         String             训练天数          是

    5      xunlianjihua_text           String             训练内容          是
...
表4.8训练进度表
  ...
  ------ ------------------- -------------------- ------------------- --------
   序号         列名               数据类型              说明          允许空
    ...
    2      xunlianjihua_id         Integer             训练计划          是

    3         yonghu_id            Integer               球员            是

    4     xunlianjindu_text         String             训练进度          是
...
表4.10球员表
  ...
  ------ ------------------ -------------------- ------------------- --------
   序号         列名              数据类型              说明          允许空
    ...
    2       yonghu_name            String             球员姓名          是

    3       yonghu_phone           String              手机号           是
...
球队训练信息管理系统的目标用户最终面向的是系统的功能，所以检验本系统的功能也是一项重要任务，只有该系统的功能都能保证正常使用，才会吸引更多的用户来使用。
...
通过测试，可以发现球队训练信息管理系统具备完善的功能，同时具备易操作性和易维护性的特点，在系统安全性上也有比较健全的数据保护机制。至此，球队训练信息管理系统可以投入使用。

# 结 论

之前在课本上学习过软件开发的知识，包括开发流程的了解，开发技术的学习，还有如何进行系统需求分析与设计，老师也选取了比较有代表性的管理系统在课堂上进行讲解，并在课后布置作业让我们练习，期间老师也让我们也对系统实现与测试进行了了解。所以面对本次球队训练信息管理系统的开发制作，我的脑海里还是有比较清晰的思路。在系统制作中，以及配套文档的编写中，我都是按照软件开发流程一步步开展我的工作。
...
从文档的角度来讲，在完成球队训练信息管理系统制作后，对其制作过程需要进行描述，包括如何进行的需求分析，如何完成系统的设计，以及实现的系统功能的运行效果等都要进行描述。这期间我也花费了将近一个月时间来完成，为了达到学院要求的文档排版标准，我也多次在导师建议下，学习办公软件的使用，还有排版技巧。功夫不负有心人，我编写的文档在面临多次编写修改后终于完成了。
...
1.  卢鸿,朱隆尹.球队训练信息管理系统查询系统优化设计及应用\[J\].南昌大学学报(理科版),2016,36(04):402-404.

    \[2\]高岳,张腾.基于基站定位的球队训练信息管理系统查询系统设计\[J\].无线互联科技,2015(03):32-35.

\[3\]钱佳加,唐海江,夏莎莎,姚宇杰,张祥杰.球队训练信息管理系统查询系统的设计与实现\[J\].现代商业,2015(17):67-68.
...
### 0211springboot球队训练信息管理系统 项目图片
![图片](/images/0211springbootimg_001.jpg)
![图片](/images/0211springbootimg_003.jpg)
![图片](/images/0211springbootimg_002.jpg)
![图片](/images/0211springbootimg_004.jpg)
