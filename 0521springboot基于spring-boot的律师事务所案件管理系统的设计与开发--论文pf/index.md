...
# [0521springboot基于Spring Boot的律师事务所案件管理系统的设计与开发--论文pf](https://github.com/GraduationProject-springboot/0521springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1eMbYemE1U&bvid=BV1eMbYemE1U&cid=500001617124121&p=23)
...
**关键词**：律师事务所案件管理系统；SpringBoot框架，系统分析，数据库设计
...
![](media/image5.jpeg){width="2.9166666666666665in"
...
![](media/image6.jpeg){width="2.9166666666666665in"
height="1.5270833333333333in"}
...
表4.1案源信息表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    2     anyuan_uuid_number         String             案源编号          是

    3        anyuan_name             String             案源名称          是

    4        anyuan_types           Integer             案源类型          是

    5         yonghu_id             Integer               客户            是

    6       anyuan_content           String             案源介绍          是

    7     anyuan_yesno_types        Integer             收案审批          是

    8     anyuan_yesno_text          String             审批回复          是

    9       anyuan_delete           Integer             逻辑删除          是
...
表4.2案件信息表
  ...
  ------ -------------------------- -------------------- ------------------- --------
   序号             列名                  数据类型              说明          允许空
    ...
    2            anyuan_id                Integer               案源            是

    3             lvshi_id                Integer               律师            是

    4        anyuan_yuyue_text             String             案源分析          是

    5          liucheng_text               String               流程            是

    6            jilu_text                 String             交接记录          是

    7          jingfei_price             BigDecimal           办案经费          是

    8        anyuan_yuyue_file             String               材料            是

    9        anyuan_yuyue_types           Integer             案件类型          是

    10    anyuan_yuyue_yesno_types        Integer             结案审批          是

    11    anyuan_yuyue_yesno_text          String             审批回复          是

    12    anyuan_yuyue_shenhe_time          Date              审核时间          是

    13          insert_time                 Date              收案时间          是
...
表4.4结案评论表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    3     jieanpingluen_text         String             评论内容          是

    4        insert_time              Date              评论时间          是
...
表4.5事务所意见表
  ...
  ------ ----------------- -------------------- ------------------- --------
   序号        列名              数据类型              说明          允许空
    ...
    3     kehuyijian_text         String             意见内容          是
...
表4.6律师表
  ...
  ------ ----------------- -------------------- ------------------- --------
   序号        列名              数据类型              说明          允许空
    ...
    2       lvshi_name            String             律师名称          是

    3       lvshi_photo           String               头像            是

    4       lvshi_phone           String            律师手机号         是

    5     lvshi_id_number         String           律师身份证号        是

    6       lvshi_email           String               邮箱            是

    7      lvshi_delete          Integer               假删            是
...
表4.7律师意见表
  ...
  ------ ------------------------ -------------------- ------------------- --------
   序号            列名                 数据类型              说明          允许空
    ...
    2            lvshi_id               Integer               律师            是

    3           yonghu_id               Integer               用户            是

    4     lvshi_commentback_text         String             意见内容          是
...
表4.9客户表
  ...
  ------ ------------------ -------------------- ------------------- --------
   序号         列名              数据类型              说明          允许空
    ...
    2       yonghu_name            String             客户名称          是

    3       yonghu_photo           String               头像            是

    4       yonghu_phone           String            客户手机号         是

    5     yonghu_id_number         String           客户身份证号        是
...
如图5.2显示的就是律师信息管理页面，此页面提供给管理员的功能有：查看已发布的律师信息数据，修改律师信息，律师信息作废，即可删除，还进行了对律师信息名称的模糊查询
律师信息信息的类型查询等等一些条件。

![](media/image14.png){width="5.770138888888889in"
...
律师事务所案件管理系统的目标用户最终面向的是系统的功能，所以检验本系统的功能也是一项重要任务，只有该系统的功能都能保证正常使用，才会吸引更多的用户来使用。
...
通过测试，可以发现律师事务所案件管理系统具备完善的功能，同时具备易操作性和易维护性的特点，在系统安全性上也有比较健全的数据保护机制。至此，律师事务所案件管理系统可以投入使用。

# 结 论

之前在课本上学习过软件开发的知识，包括开发流程的了解，开发技术的学习，还有如何进行系统需求分析与设计，老师也选取了比较有代表性的管理系统在课堂上进行讲解，并在课后布置作业让我们练习，期间老师也让我们也对系统实现与测试进行了了解。所以面对本次律师事务所案件管理系统的开发制作，我的脑海里还是有比较清晰的思路。在系统制作中，以及配套文档的编写中，我都是按照软件开发流程一步步开展我的工作。
...
从文档的角度来讲，在完成律师事务所案件管理系统制作后，对其制作过程需要进行描述，包括如何进行的需求分析，如何完成系统的设计，以及实现的系统功能的运行效果等都要进行描述。这期间我也花费了将近一个月时间来完成，为了达到学院要求的文档排版标准，我也多次在导师建议下，学习办公软件的使用，还有排版技巧。功夫不负有心人，我编写的文档在面临多次编写修改后终于完成了。
...
1.  卢鸿,朱隆尹.律师事务所案件管理系统查询系统优化设计及应用\[J\].南昌大学学报(理科版),2016,36(04):402-404.

    \[2\]高岳,张腾.基于基站定位的律师事务所案件管理系统查询系统设计\[J\].无线互联科技,2015(03):32-35.

\[3\]钱佳加,唐海江,夏莎莎,姚宇杰,张祥杰.律师事务所案件管理系统查询系统的设计与实现\[J\].现代商业,2015(17):67-68.
...
### 0521springboot基于Spring Boot的律师事务所案件管理系统的设计与开发--论文pf 项目图片
![图片](/images/0521springbootimg_014.jpg)
![图片](/images/0521springbootimg_001.jpg)
![图片](/images/0521springbootimg_003.jpg)
![图片](/images/0521springbootimg_002.jpg)
![图片](/images/0521springbootimg_012.jpg)
![图片](/images/0521springbootimg_006.jpg)
![图片](/images/0521springbootimg_007.jpg)
![图片](/images/0521springbootimg_013.jpg)
![图片](/images/0521springbootimg_005.jpg)
![图片](/images/0521springbootimg_011.jpg)
![图片](/images/0521springbootimg_010.jpg)
![图片](/images/0521springbootimg_004.jpg)
![图片](/images/0521springbootimg_009.jpg)
![图片](/images/0521springbootimg_008.jpg)
