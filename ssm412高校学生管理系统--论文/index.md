...
# [ssm412高校学生管理系统--论文](https://github.com/GraduationProject-springboot/)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=12)

# 绪论
## 1.1 研究背景
现在大家正处于互联网加的时代，这个时代它就是一个信息内容无比丰富，信息处理与管理变得越加高效的网络化的时代，这个时代让大家的生活不仅变得更加地便利化，也让时间变得更加地宝贵化，因为每天的每分钟，每秒钟这些时间都能让人们处理大批量的日常事务，这些场景，是之前的手工模式无法与之相抗衡的。对于校园活动信息的管理来说，传统的通过纸质文档记录信息的方式已经落后了，依靠手工管理这些信息，不仅花费较长的工作时间，在对记录各种信息的文档进行信息统计以及信息核对操作时，也不能及时保证信息的准确性，基于这样的办公低效率环境下，对于校园活动信息的处理就要提出新的解决方案。因为这个时代的信息一直都在高速发展，要是不抱着发展的观念看待事情，极有可能被这个市场快速遗忘，甚至被无情地淘汰掉。所以尽早开发一款高校学生管理系统进行信息的快速处理，既跟上了时代的发展脚步，也能让自己的核心竞争力有所提升。
## 1.2目的和意义
互联网加的时代一方面是加快信息的发展，另一方面也是对传统行业进行筛选，能够继续发展的，肯定是那些能够充分运用互联网技术进行自身升级改革的行业。那些停步不前的行业只能就此结束，进而被大家所遗忘。这次设计出来的高校学生管理系统，它不仅能够让管理人员在信息增加，信息的编辑等事务处理上，节省很多的时间，也会砍掉一部分的人工成本，节省不必要开支的资金。另外，此系统的操作界面是可视化的界面，管理人员无需付费培训就能尽快上手。高校学生管理系统的开发意义如下：
...
## 1.3 论文结构安排
编写高校学生管理系统相对应的论文，其实就是对开发完成的程序进行再次解读的过程。本论文从七个方面的内容讲解了开发的程序，具体内容如下：
...
# 3 系统分析
## 3.1系统可行性分析
需要使用大部分精力开发的高校学生管理系统为了充分降低开发风险，特意在开发之前进行可行性分析这个验证系统开发是否可行的步骤。本文就会从技术角度，经济角度，还有用户使用的程序的运行角度进行综合阐述。
### 3.1.1 技术可行性分析
...
## 3.3系统流程分析
管理员假如要操作系统提供的功能，那么管理员就要在系统的登录界面，填写管理员登录的账号信息，填写相应的密码信息，管理员需要保证这两者能够验证身份的账号以及密码信息的正确性，这样管理员就可以通过登录界面进入系统后台操作界面。图3.1就是开发的程序软件高校学生管理系统它的操作流程图。

![](/images/0400ssm/ssm412/blog.001.png)

图3.1 系统操作流程图
### 3.3.1系统登录流程
高校学生管理系统的登录流程，针对的角色就是操作员的操作角色。在登录界面需要的必填信息就是账号信息，配上登录的密码信息就能登录高校学生管理系统，需要注意的就是必填的账号信息和登录密码信息，都需要进行验证，系统会判断账号还有填写的密码信息的正确性，只有这两者信息都正确了，就能成功登录高校学生管理系统了。系统登录流程图如下图。

![](/images/0400ssm/ssm412/blog.002.png)

图3.2　系统登录流程图
### 3.3.2信息添加流程
用户在添加信息的界面填写的任何数据信息也是需要验证的，系统会判断用户填写信息的格式还有数据信息是不是合法信息，如果用户填写的信息是合法内容，系统就会在数据库对应的数据表里面添加信息。添加信息流程如下图。

![](/images/0400ssm/ssm412/blog.003.png)

图3.3 添加信息流程图
### 3.3.3信息删除流程
对于那些已经失效的信息，需要用户及时进行删除，这样有利于腾出空间存放其他信息。删除信息也是先从数据库对应数据表里面删除数据，接着就是更新数据表的信息。这样删除的数据，在用户操作界面就查看不到了。信息删除流程如下图所示。

![](/images/0400ssm/ssm412/blog.004.png)

图3.4 信息删除流程图

# 4 系统设计
## 4.1系统概要设计
高校学生管理系统并没有使用C/S结构，而是基于网络浏览器的方式去访问服务器，进而获取需要的数据信息，这种依靠浏览器进行数据访问的模式就是现在用得比较广泛的适用于广域网并且没有网速限制要求的B/S结构，图4.1就是开发出来的程序工作原理图。

![](/images/0400ssm/ssm412/blog.005.png)

图4.1 程序工作的原理图
## 4.2系统功能结构设计
下图就是系统功能结构图。

![](/images/0400ssm/ssm412/blog.006.png)
...
下面就展示高校学生管理系统的实体E-R图。

（1）下图就是管理员实体E-R图

![](/images/0400ssm/ssm412/blog.007.png)

图4.7 管理员实体E-R图

（2）下图就是教师实体E-R图

![](/images/0400ssm/ssm412/blog.008.png)

图4.8 教师实体E-R图

（3）下图就是院系班级实体E-R图

![](/images/0400ssm/ssm412/blog.009.png)

图4.9 院系班级实体E-R图
### 4.3.2 数据库表结构设计
本次程序开发选用的数据库管理工具是MySQL数据管理工具，使用它存放数据也需要创建程序对应的数据库文件，并命名刚创建的数据库文件，有了数据库也需要创建各种数据表来充实数据库，在数据表的创建中，不仅需要对数据表命名，也需要对数据表的字段进行设计，包括每个数据表里面需要设置的字段名称，字段对应的数据类型信息，字段的主键设置这个也是不可缺少的，因为每个数据表里面的主键就是标记着这个数据表跟其他数据表相区分的唯一标志。就相当于生活中的每个人都有姓名，但是上网搜索自己的名字，会发现全国上下有很多人的名字跟自己的名字一模一样，包括姓氏以及名字，区分每个人的唯一信息就是每个人的身份证号信息，主键在数据表里面也是起着这样的重要作用。下面就介绍本次开发的程序高校学生管理系统的数据表结构信息。
...
|3|xuejihao|String|学籍号|是|
|4|xueshengxingming|String|学生姓名|是|
|5|mima|String|密码|是|
|6|nianji|String|年级|是|
|7|banji|String|班级|是|
|8|xingbie|String|性别|是|
|9|zhuanye|String|专业|是|
|10|lianxidianhua|String|联系电话|是|
|11|youxiang|String|邮箱|是|
|12|shenfenzheng|String|身份证|是|
|13|chushengriqi|date|出生日期|是|
|14|minzu|String|民族|是|
|15|xueyuan|String|学院|是|
|16|ruxueshijian|date|入学时间|是|
|17|xuezhi|String|学制|是|
|18|zhengzhimianmao|String|政治面貌|是|
|19|touxiang|String|头像|是|
4.2教师表
...
|9|jiaoshiyouxiang|String|教师邮箱|是|
4.3学生成绩表
...
|5|jiaoshigonghao|String|教师工号|是|
|6|jiaoshixingming|String|教师姓名|是|
|7|xuejihao|String|学籍号|是|
|8|xueshengxingming|String|学生姓名|是|
|9|dengji|String|等级|是|
|10|chengji|float|成绩|是|
|11|jiaoshipingyu|String|教师评语|是|
|12|dengjishijian|date|登记时间|是|
4.4收藏表表
...
|8|kechengxiangqing|String|课程详情|是|
4.6课表信息表
...
|2|addtime|Date|创建时间|是|
|3|banji|String|班级|是|
|4|xingqi|String|星期|是|
|5|kechengyi|String|课程一|是|
|6|kechenger|String|课程二|是|
|7|kechengsan|String|课程三|是|
|8|kechengsi|String|课程四|是|
|9|kechengwu|String|课程五|是|
|10|kechengliu|String|课程六|是|
|11|kechengqi|String|课程七|是|
...
|9|leixing|String|类型|是|
|10|jutishijian|date|具体时间|是|
|11|xiangxineirong|String|详细内容|是|
...
|3|kechengmingcheng|String|课程名称|是|
|4|xuefen|float|学分|是|
|5|zhuangtai|String|状态|是|
|6|peiyangjihua|String|培养计划|是|
4.10学生请假表
...
|3|xuejihao|String|学籍号|是|
|4|xueshengxingming|String|学生姓名|是|
|5|jutishijian|date|具体时间|是|
|6|qingjiatianshu|String|请假天数|是|
|7|shenqingshijian|datetime|申请时间|是|
|8|liyou|String|理由|是|
...
|2|addtime|Date|创建时间|是|
|3|banji|String|班级|是|
|4|zongrenshu|String|总人数|是|
|5|mingdan|String|名单|是|
...
![](/images/0400ssm/ssm412/blog.010.png)

图5.1 教师信息界面
### 5.1.2 学生信息管理
管理员可以对学生信息进行添加修改删除操作。具体界面如图5.2所示。

![](/images/0400ssm/ssm412/blog.011.png)

图5.2 学生信息管理界面
## 5.2 学生模块的实现
### 5.2.1 学生请假管理
学生可以提交自己的请假信息，提交后教师可以进行审批。界面如下图所示：

![](/images/0400ssm/ssm412/blog.012.png)

图5.3 学生请假管理界面

### 5.2.2 课表信息管理
学生可以点击课表信息管理查看课表信息，还可以根据班级以及日期进行查询操作。界面如下图所示：

![](/images/0400ssm/ssm412/blog.013.png)

图5.4 课表信息管理界面
## 5.3 教师模块的实现
### 5.3.1 学生请假审核
教师可以对学生请假信息进行审核操作。界面如下图所示：

![](/images/0400ssm/ssm412/blog.014.png)

图5.5 学生请假审核
### 5.3.2 学生成绩管理
教师可以对学生成绩进行添加，修改，删除，查询操作。界面如下图所示：

![](/images/0400ssm/ssm412/blog.015.png)
