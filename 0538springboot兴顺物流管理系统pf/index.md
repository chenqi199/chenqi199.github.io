...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1eMbYemE1U&bvid=BV1eMbYemE1U&cid=500001617128469&p=39)
...
[5.1.2 物流资讯信息管理 14](#物流资讯信息管理)

[5.1.3 物流资讯类型管理 15](#物流资讯类型管理)
...
[6.4.2 添加物流资讯类别测试 17](#添加物流资讯类别测试)
...
AB公司在很早之前就收购过一个关系型数据库，它是现在的Mysql数据库。所以兴顺物流管理系统后台数据库使用的是MySQL进行数据库方面的开发工作的，MySQL它是微软开发的一款平台软件，这个软件可以给用户提供高效率的智能数据，并且数据信息还是很可靠，使用它进行数据存储可以满足大众企业管理各种各样的数据信息的需求。MySQL在MySQL版本里面它是最全面的，也是最强大的开发平台，
...
![](media/image3.jpeg){width="6.095833333333333in"
...
表4.3物流资讯表
  ...
  ------ ----------------- -------------------- ------------------- --------
   序号        列名              数据类型              说明          允许空
    ...
    2      gonggao_name           String           物流资讯名称        是

    3      gonggao_types         Integer           物流资讯类型        是

    4       insert_time            Date          物流资讯发布时间      是

    5     gonggao_content         String           物流资讯详情        是
...
表4.4货运表
  ...
  ------ ------------------------ -------------------- ------------------- --------
   序号            列名                 数据类型              说明          允许空
    ...
    4         jiashiyuan_id             Integer              驾驶员           是

    5          cheliang_id              Integer               车辆            是

    6       huoyun_uuid_number           String             订单编号          是

    7          huoyun_name               String             物品标题          是

    8          wupin_types              Integer             物品类型          是

    9          huoyun_types             Integer             货运类型          是

    10      huoyun_zhongliang          BigDecimal           物品重量          是

    11         huoyun_chang            BigDecimal            物品长           是

    12         huoyun_kuan             BigDecimal            物品宽           是

    13          huoyun_gao             BigDecimal            物品高           是

    14         huoyun_jine             BigDecimal           货运金额          是

    15        huoyun_chufadi             String              出发地           是

    16          chufa_time                Date              出发时间          是

    17        huoyun_tujing              String              途径地           是

    18        huoyun_didadi              String              抵达地           是

    19         songda_time                Date              送达时间          是

    20        wupin_content              String             物品备注          是

    21        huoyun_content             String             货运备注          是

    22         yushu_types              Integer             运输状态          是

    23    huoyun_pingjia_content         String               评价            是

    24         insert_time                Date              录入时间          是
...
表4.5驾驶员表
  ...
  ------ -------------------------- -------------------- ------------------- --------
   序号             列名                  数据类型              说明          允许空
    ...
    2         jiashiyuan_name              String            驾驶员姓名         是

    3         jiashiyuan_phone             String           驾驶员手机号        是

    4       jiashiyuan_id_number           String          驾驶员身份证号       是

    5         jiashiyuan_photo             String            驾驶员照片         是

    6     jiashiyuan_jiazhao_photo         String             驾照照片          是

    7        jiashiyuan_jialing            String               驾龄            是

    8            sex_types                Integer               性别            是

    9         jiashiyuan_email             String            驾驶员邮箱         是

    10       jiashiyuan_content            String            驾驶员介绍         是
...
表4.6客户表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    2       kehu_name            String             客户姓名          是

    3       kehu_phone           String            客户手机号         是

    4     kehu_id_number         String           客户身份证号        是

    5       kehu_photo           String             客户头像          是

    6       kehu_email           String             客户邮箱          是
...
表4.7通知板表
  ...
  ------ ---------------- -------------------- ------------------- --------
   序号        列名             数据类型              说明          允许空
    ...
    3      tongzhi_name          String             通知标题          是

    4     tongzhi_types         Integer             通知类型          是

    5      tongzhi_text          String             通知内容          是

    6      insert_time            Date              通知时间          是
...
![](media/image11.png){width="6.085416666666666in"
...
兴顺物流管理系统的测试选用的测试平台是IDEA平台环境，测试时首先需要用户打开MySQL数据库进行数据库文件的附加操作，然后打开IDEA，选择文件打开网站，把兴顺物流管理系统的程序添加进入IDEA平台中，接着把文件部署到tomcat服务器里面，最后运行程序，这时用户可以操作系统里面的各个功能，看看程序有没有达到用户的要求。
...
管理员可以在添加物流资讯类别界面输入物流资讯类别名称信息，如果数据信息为空，系统会给出相应提示。只有所有的数据信息都合理输入，管理员才能完成物流资讯类别的添加操作。测试数据见下表：
...
表6.2添加物流资讯类别测试表
  ...
  添加物流资讯类别   管理员登陆后点击添加物流资讯类别按钮，添加物流资讯类别信息   类别名称为空       添加失败，弹出提示按钮   添加失败，提示请填写内容
...
经过此次对兴顺物流管理系统的综合性测试，我们不难发现程序的功能并没有出现明显的逻辑性错误，用户在进行功能操作时，程序基本能根据用户操作情况给出相应的反馈。程序质量以及可靠性在系统的反复测试中都经过了严格检验，程序投入生活使用完全没有问题。

# 结 论

兴顺物流管理系统的开发制作，从题目确定到成品完成，自己投入的精力与心血是非常多的。从兴顺物流管理系统的前台页面实现，到兴顺物流管理系统的后台代码的编辑，我用到的软件包括了数据库软件Mysql，Java开发工具IDEA，办公软件Office，浏览器软件Fireworks，图像处理软件Photoshop等，这也是我第一次使用Java语言，开发的这个比较简单的兴顺物流管理系统。

开发兴顺物流管理系统让我明白知识理论与现实实践的差距还是挺大的，很多时候，我们在学习课程知识的时候，都感觉书本上的内容，老师一点就通，然而这次毕设就真正考验了我，原来理论知识和现实实践完全是两码事，开发兴顺物流管理系统一方面让我对在校学习的知识起了一个巩固作用，另一方面让我明白独立学习知识的重要性。在毕设作品完成期间，我才知道要开发一款软件需要经历很多繁琐的步骤，从开始的选题到系统需求分析以及功能板块的设计，还有系统详细实现以及系统测试，每个环节看似简单，其实暗藏很多知识点，这些都需要自己去网上或者是学校的图书馆查阅相对应的资料。每个环节都需要认真对待，要是系统需求分析没有做好，那么在系统设计与实现过程中就会出现很多错误，要么功能不全，要么就是程序开发出来根本不符合用户要求，所以程序的开发是一个知识再造的过程，一个人独自开发完一款软件也就深知这其间的各种不易了。
...
\[13\]刘小华,化琼珍.兴顺物流管理系统的构建\[J\].物流技术与应用,2015(03):100-101.

\[14\]王锋.兴顺物流管理系统构建的研究\[J\].商场现代化,2016(24):35-36.

\[15\]王育才.兴顺物流管理系统构建的分析\[J\].中国管理信息化,2017,20(22):54-55.
...
### 0538springboot兴顺物流管理系统pf 项目图片
![图片](/images/0538springbootimg_001.jpg)
![图片](/images/0538springbootimg_003.jpg)
![图片](/images/0538springbootimg_002.jpg)
![图片](/images/0538springbootimg_012.jpg)
![图片](/images/0538springbootimg_006.jpg)
![图片](/images/0538springbootimg_007.jpg)
![图片](/images/0538springbootimg_005.jpg)
![图片](/images/0538springbootimg_011.jpg)
![图片](/images/0538springbootimg_010.jpg)
![图片](/images/0538springbootimg_004.jpg)
![图片](/images/0538springbootimg_009.jpg)
![图片](/images/0538springbootimg_008.jpg)
