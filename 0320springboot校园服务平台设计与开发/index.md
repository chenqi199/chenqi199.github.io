...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1T1bpekEK7&bvid=BV1T1bpekEK7&cid=500001612673098&p=19)
...
表4.3接单员表
  ...
  ------ ------------------ -------------------- ------------------- --------
   序号         列名              数据类型              说明          允许空
    ...
    2     jiedanyuan_name          String            接单员姓名         是
...
表4.5跑腿任务表
  ...
  ------ -------------------------- -------------------- ------------------- --------
   序号             列名                  数据类型              说明          允许空
    ...
    2     paotuirenwu_uuid_number          String             任务编号          是

    3         paotuirenwu_name             String             任务标题          是

    4        paotuirenwu_types            Integer             任务类型          是

    5      paotuirenwu_new_money         BigDecimal             单价            是

    6            yonghu_id                Integer               用户            是

    7         shouhuodizhi_id             Integer               地址            是

    8         paotuirenwu_text             String             任务备注          是

    9     paotuirenwu_status_types        Integer             任务状态          是
...
表4.6收货地址表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    3     shouhuodizhi_name          String              收货人           是

    4     shouhuodizhi_phone         String               电话            是

    5     shouhuodizhi_dizhi         String               地址            是

    6        insert_time              Date              添加时间          是

    7        update_time              Date              修改时间          是
...
表4.8用户投诉表
  ...
  ------ ------------------------- -------------------- ------------------- --------
   序号            列名                  数据类型              说明          允许空
    ...
    3        yonghutousu_name             String             投诉标题          是

    4        yonghutousu_types           Integer             投诉类型          是

    5       yonghutousu_content           String             投诉详情          是

    6     yonghutousu_yesno_types        Integer             投诉状态          是

    7     yonghutousu_yesno_text          String             投诉结果          是
...
表4.9余额变更记录表
  ...
  ------ ----------------------- -------------------- ------------------- --------
   序号           列名                 数据类型              说明          允许空
    ...
    3      yuebiangengjil_name          String               原因            是

    4     yuebiangengjil_number       BigDecimal           涉及数量          是
...
如图5.2显示的就是接单员管理页面，此页面提供给管理员的功能有：查看已发布的接单员数据，修改接单员，接单员作废，即可删除，还进行了对接单员名称的模糊查询
接单员信息的类型查询等等一些条件。
...
通过设计一些数据检验校园服务平台的功能是否根据预定进行数据输出，以此检验系统功能是否合格。接下来就选取部分功能进行检测。

### 6.1.1 登录功能测试

校园服务平台提供的服务是给规定的用户使用的，其他无关用户是无法进入的，登录功能就是检验访问者是否是符合要求的用户，及时将不符合要求的用户拒之门外。以此保证系统安全。这里以管理员身份进行检测，测试的内容见下表。
...
校园服务平台在面对正确的账号，不匹配的密码时，会有相应的反馈，其反馈结果见下图。
...
经过测试，从校园服务平台功能的角度来看，本系统各个功能在符合用户要求的情况下，也能保持运行正常。从校园服务平台性能的角度来看，本系统可以保持24小时不间断运行，而且面对用户的误操作行为，有提前设置的错误反馈机制应对，另外，本系统界面布局考虑用户阅读习惯，可以让用户短时间内获取需要的内容。总之，本系统可以投入生产，帮助用户解决实际问题，发挥其应用价值。

# 结 论

采用Java+Mysql的方式设计制作的校园服务平台，在功能上可以满足管理人员对于信息的管理需求；在界面的设计上，尽最大努力用自身的美工知识做到界面美观简洁。在操作方面上，让系统可以方便操作，同时对用户常见的误操作行为进行分析总结，并尽力在系统中避免同样的由于用户的误操作所带来的错误现象。

作为毕业设计，分配给本系统的设计与制作时间还是不足的，所以，校园服务平台还有许多需要完善的地方。
...
\[1\]江涛.接单员网上预订系统的设计与分析\[J\].现代商贸工业,2020,41(22):150.

\[2\]徐微微.晓庄连锁接单员接单员预订系统的设计\[J\].企业导报,2016(08):180-181.
...
### 0320springboot校园服务平台设计与开发 项目图片
![图片](/images/0320springbootimg_001.jpg)
![图片](/images/0320springbootimg_003.jpg)
![图片](/images/0320springbootimg_002.jpg)
![图片](/images/0320springbootimg_006.jpg)
![图片](/images/0320springbootimg_007.jpg)
![图片](/images/0320springbootimg_005.jpg)
![图片](/images/0320springbootimg_004.jpg)
