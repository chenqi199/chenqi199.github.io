...
# [0297springboot计算机学院校友网](https://github.com/GraduationProject-springboot/0297springboot)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://player.bilibili.com/player.html?isOutside=true&aid=BV1jqaLe1EbH&bvid=BV1jqaLe1EbH&cid=500001610887658&p=63)
...
表4.1分会成员表
  ...
  ------ ------------------ -------------------- ------------------- --------
   序号         列名              数据类型              说明          允许空
    ...
    2     xiaoyoufenhui_id        Integer             分会名称          是

    3      chengyuan_name          String             成员名称          是

    4     chengyuan_photo          String             成员头像          是

    5        sex_types             String               性别            是

    6     chengyuan_phone          String             联系方式          是
...
表4.6校友风采表
  ...
  ------ ------------------------ -------------------- ------------------- --------
   序号            列名                 数据类型              说明          允许空
    ...
    3      xiaoyoufengcai_name           String               标题            是

    4      xiaoyoufengcai_types         Integer             奖项类型          是

    5      xiaoyoufengcai_photo          String               封面            是

    6     xiaoyoufengcai_jiango          String             所得奖项          是

    7     xiaoyoufengcai_content         String               详情            是
...
表4.7校友分会表
  ...
  ------ ------------------------ -------------------- ------------------- --------
   序号            列名                 数据类型              说明          允许空
    ...
    2       xiaoyoufenhui_name           String             分会名称          是

    3      xiaoyoufenhui_types          Integer             分会类型          是

    4      xiaoyoufenhui_photo           String             分会封面          是

    5     xiaoyoufenhui_fuzheren         String              负责人           是

    6     xiaoyoufenhui_address          String             分会地点          是

    7     xiaoyoufenhui_content          String             分会简介          是
...
表4.9校友招聘表
  ...
  ------ ----------------------- -------------------- ------------------- --------
   序号           列名                 数据类型              说明          允许空
    ...
    11       zhaopin_content            String           招聘信息详情        是
...
如图5.2显示的就是校友分会管理页面，此页面提供给管理员的功能有：查看已发布的校友分会数据，修改校友分会，校友分会作废，即可删除，还进行了对校友分会名称的模糊查询
校友分会信息的类型查询等等一些条件。
...
通过设计一些数据检验计算机学院校友网的功能是否根据预定进行数据输出，以此检验系统功能是否合格。接下来就选取部分功能进行检测。

### 6.1.1 登录功能测试

计算机学院校友网提供的服务是给规定的用户使用的，其他无关用户是无法进入的，登录功能就是检验访问者是否是符合要求的用户，及时将不符合要求的用户拒之门外。以此保证系统安全。这里以管理员身份进行检测，测试的内容见下表。
...
计算机学院校友网在面对正确的账号，不匹配的密码时，会有相应的反馈，其反馈结果见下图。
...
经过测试，从计算机学院校友网功能的角度来看，本系统各个功能在符合用户要求的情况下，也能保持运行正常。从计算机学院校友网性能的角度来看，本系统可以保持24小时不间断运行，而且面对用户的误操作行为，有提前设置的错误反馈机制应对，另外，本系统界面布局考虑用户阅读习惯，可以让用户短时间内获取需要的内容。总之，本系统可以投入生产，帮助用户解决实际问题，发挥其应用价值。

# 结 论

采用Java+Mysql的方式设计制作的计算机学院校友网，在功能上可以满足管理人员对于信息的管理需求；在界面的设计上，尽最大努力用自身的美工知识做到界面美观简洁。在操作方面上，让系统可以方便操作，同时对用户常见的误操作行为进行分析总结，并尽力在系统中避免同样的由于用户的误操作所带来的错误现象。

作为毕业设计，分配给本系统的设计与制作时间还是不足的，所以，计算机学院校友网还有许多需要完善的地方。
...
\[1\]江涛.校友分会网上预订系统的设计与分析\[J\].现代商贸工业,2020,41(22):150.

\[2\]徐微微.晓庄连锁校友分会校友分会预订系统的设计\[J\].企业导报,2016(08):180-181.
...
### 0297springboot计算机学院校友网 项目图片
![图片](/images/0297springbootimg_001.jpg)
![图片](/images/0297springbootimg_003.jpg)
![图片](/images/0297springbootimg_002.jpg)
![图片](/images/0297springbootimg_006.jpg)
![图片](/images/0297springbootimg_007.jpg)
![图片](/images/0297springbootimg_005.jpg)
![图片](/images/0297springbootimg_004.jpg)
