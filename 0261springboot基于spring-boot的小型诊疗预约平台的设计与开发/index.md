...
[]{#refheading___toc671468916 .anchor}

[目 录 III](#refheading___toc671468916)
...
[4.4 数据库设计 13](#__RefHeading___Toc432558317)

[4.4.1 数据库E-R图设计 13](#__RefHeading___Toc775488724)
...
[6 系统测试 20](#__RefHeading___Toc857364713)
...
[致 谢 24](#__RefHeading___Toc1887483601)
...
表4.1病例表
  ...
  ------ -------------------- -------------------- ------------------- --------
   序号          列名               数据类型              说明          允许空
    ...
    6       bingli_address           String              现住址           是

    7       bingli_jiguan            String               籍贯            是

    8       bingli_hunyin            String             婚姻状况          是

    9       zhusu_content            String               主诉            是

    10    jiwangshi_content          String              既往史           是

    11      geren_content            String              个人史           是

    12     yichuan_content           String            家族遗传史         是

    13     zhenduan_content          String             医生诊断          是

    14     yaofang_content           String             开具药方          是

    15       kanbing_time             Date              看病时间          是

    16       jiaofei_jine          BigDecimal           缴费金额          是

    17       insert_time              Date              录入时间          是

    18      jiaofei_types           Integer             是否缴费          是
...
表4.6药品表
  ...
  ------ --------------------- -------------------- ------------------- --------
   序号          列名                数据类型              说明          允许空
    ...
    7       yaopin_content            String           药品详细介绍        是
...
表4.7医生表
  ...
  ------ --------------------- -------------------- ------------------- --------
   序号          列名                数据类型              说明          允许空
    ...
    7      yisheng_shanchang          String             医生擅长          是
    ...
    13      yisheng_content           String           医生详细介绍        是
...
表4.9医生留言表
  ...
  ------ --------------------- -------------------- ------------------- --------
   序号          列名                数据类型              说明          允许空
    ...
    4     yisheng_liuyan_text         String             留言内容          是
...
表4.10医生预约挂号订单表
  ...
  ------ ----------------------------- -------------------- ------------------- --------
   序号              列名                    数据类型              说明          允许空
    ...
    2      yisheng_order_uuid_number          String           预约挂号编号        是

    3             yisheng_id                 Integer               医生            是

    4             huanzhe_id                 Integer               患者            是

    5      yisheng_order_true_price         BigDecimal           实付价格          是

    6         yisheng_order_types            Integer             挂号状态          是

    7            kanbing_time                  Date            预约看病日期        是

    8          shijianduan_types             Integer            看病时间段         是

    9     yisheng_order_payment_types        Integer             支付类型          是

    10            insert_time                  Date              挂号时间          是
...
![](media/image17.png){width="5.759722222222222in" height="3.5in"}
...
![](media/image19.png){width="5.759722222222222in" height="3.5in"}

图5.4 公告类型列表页面

[]{#__RefHeading___Toc857364713 .anchor}
...
小型诊疗预约平台需要管理员添加公告类型信息，公告类型名称和公告类型编码都是程序设置的必填数据，而且添加的公告类型编码也不能够是数据库里面的存在的数据。对添加公告类型功能测试时，测试数据在下表展示。
...
第一点，可以确定开发出来的小型诊疗预约平台是符合同类型系统的要求；

第二点，用户要求小型诊疗预约平台需要具备的功能都已开发完成并能够正常使用；

第三点，小型诊疗预约平台界面简洁美观，操作流程清晰明了；

第四点，小型诊疗预约平台的安全性，以及运行性能符合开发要求。

至此，小型诊疗预约平台可以确保所有功能能够按照预期目标进行运行，可以正式投入生活中运行使用！
...
1.  李德华.基于SSM技术的小型诊疗预约平台的研究与实现\[J\].信息与电脑(理论版),2019,31(17):51-53.

    \[2\]屈敬华,王晓孟.小型诊疗预约平台的设计与实现\[J\].计算机时代,2019(01):43-45.
...
### 0261springboot基于spring boot的小型诊疗预约平台的设计与开发 项目图片
