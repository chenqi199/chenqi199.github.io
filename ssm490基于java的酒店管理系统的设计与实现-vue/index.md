# ssm490基于JAVA的酒店管理系统的设计与实现+vue


# [项目清单 包安装运行](http://chenqi1990.site) 官网地址 http://chenqi1990.site

# [ssm490基于JAVA的酒店管理系统的设计与实现+vue](https://github.com/GraduationProject-springboot/)
...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=88)
...
# 第2章 开发环境与技术
酒店管理系统需要提前对功能进行调查分析，并且根据分析结果进行技术上的辩证，可以选择更合理的开发技术以及开发工具。
## 2.1 SSM框架
...
### 3.1.1 经济可行性
开发酒店管理系统，存在经济上面的支持，必须经济上的支持才可以有序进行，比如是否需要额外购买开发工具，购买开发电脑，或者从开发技术上是否是免费开源，达到的效果上面是否支出与收入不匹配，这些都需要进行分析。由于开发本系统，开发技术开源且稳定，电脑用正在使用的电脑就能满足，不需要太多的经济支出就可以达到目的，经济可行性通过。
### 3.1.2 技术可行性
酒店管理系统软件主要用到的一些工具，而这些工具正好在学校就使用过，并且网上有很多免费的并且符合的开发工具，技术方面也学过，并且有图书馆的资源和网络资源可以充分利用。所以在技术角度上面来分析是可以的。
### 3.1.3 操作可行性
...
酒店管理系统的操作者有管理员，分析的管理员功能将使用用例图进行展示。图3.4展示了管理员用例图。管理员管理房间，用户预定房间的信息，用户对房间发布的留言，新闻信息，用户之间交流的论坛帖子信息等。

![](/images/0400ssm/ssm490/blog.004.png)

图3.4 管理员用例图

酒店管理系统的操作者有用户，分析的用户的功能将使用用例图进行展示。图3.5展示了用户用例图。用户预定房间，管理预定房间的信息，查看新闻，在论坛模块发帖或回帖等。

![](/images/0400ssm/ssm490/blog.005.png)
...
管理员具备的详细功能将参照最终的设计结果，即图4.1所示的管理员功能结构图。其中管理员管理房间，用户预定房间的信息，用户对房间发布的留言，新闻信息，用户之间交流的论坛帖子信息等。

![](/images/0400ssm/ssm490/blog.006.png)

图4.1 管理员功能结构图

用户具备的详细功能将参照最终的设计结果，即图4.2所示的用户功能结构图。其中用户预定房间，管理预定房间的信息，查看新闻，在论坛模块发帖或回帖等。

![](/images/0400ssm/ssm490/blog.007.png)
...
（1）把房间具备的属性通过属性图进行展示，绘制的属性图见图4.4。

![](/images/0400ssm/ssm490/blog.008.png)

图4.4 房间实体属性图

（2）把新闻具备的属性通过属性图进行展示，绘制的属性图见图4.5。

![](/images/0400ssm/ssm490/blog.009.png)

图4.5 新闻实体属性图

（3）把用户具备的属性通过属性图进行展示，绘制的属性图见图4.6。

![](/images/0400ssm/ssm490/blog.010.png)

图4.6 用户实体属性图

（4）把房间留言具备的属性通过属性图进行展示，绘制的属性图见图4.7。

![](/images/0400ssm/ssm490/blog.011.png)

图4.7 房间留言实体属性图

（5）上述实体间关系见图4.8。

![](/images/0400ssm/ssm490/blog.012.png)
...
|fangjian\_address|所在区域|varchar(200)|是|
|fangjian\_peizhi|房间配置|varchar(200)|是|
|fangjian\_photo|房间图片|varchar(200)|是|
|fangjian\_new\_money|价格/天|decimal(10,2)|是|
|status\_types|房间状态|int(11)|是|
|fangjian\_content|商品详情|text|是|
...
|fangjianyuding\_time|预定时间|date|是|
|update\_time|退房时间|date|是|
|fangjianyuding\_types|预定状态|int(11)|是|
|create\_time|创建时间|timestamp|是|
表4.3 房间留言表
...
|fangjian\_id|房间|int(11)|是|
|yonghu\_id|用户|int(11)|是|
|fangjian\_liuyan\_text|留言内容|text|是|
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(11)|否|
|news\_name|新闻信息标题 |varchar(200)|是|
|news\_types|新闻信息类型 |int(11)|是|
|news\_photo|新闻信息图片|varchar(200)|是|
|insert\_time|新闻信息时间|timestamp|是|
|news\_content|新闻信息详情|text|是|
...
## 5.1 管理员功能实现
### 5.1.1 房间信息管理
实现房间信息管理功能，其界面运行的效果图见图5.1。在该界面，管理员对房间状态进行查看，更改房间配置，所在区域，房间图片等资料，删除房间信息，同时可以在本界面点击新增按钮新增房间。

![](/images/0400ssm/ssm490/blog.013.png)

图5.1 房间信息管理界面
### 5.1.2 房间预定管理
实现房间预定管理功能，其界面运行的效果图见图5.2。在该界面，管理员对用户预定的房间信息进行查询并管理，可以为用户已入住的房间办理退房等操作。

![](/images/0400ssm/ssm490/blog.014.png)

图5.2 房间预定管理界面
### 5.1.3 论坛管理
实现论坛管理功能，其界面运行的效果图见图5.3。在该界面，管理员对用户的发帖以及回帖的帖子内容进行查看，能够删除有必要删除的论坛帖子信息。

![](/images/0400ssm/ssm490/blog.015.png)

图5.3 论坛管理界面
### 5.1.4 用户管理
实现用户管理功能，其界面运行的效果图见图5.4。在该界面，管理员对用户的身份证号，手机号等信息进行更改，能够删除指定用户，可以在本界面点击新增按钮新增用户信息。

![](/images/0400ssm/ssm490/blog.016.png)

图5.4 用户管理界面
### 5.1.5 新闻信息管理
实现新闻信息管理功能，其界面运行的效果图见图5.5。在该界面，管理员增加，删除，更改新闻相关信息。

![](/images/0400ssm/ssm490/blog.017.png)

图5.5 新闻信息管理界面
## 5.2 用户功能实现
### 5.2.1 房间信息
实现房间信息功能，其界面运行的效果图见图5.6。在该界面，用户预定房间，在本界面的下方对该房间发布留言等。

![](/images/0400ssm/ssm490/blog.018.png)

图5.6 房间信息界面
### 5.2.2 在线论坛
实现在线论坛功能，其界面运行的效果图见图5.7。在该界面，用户可以查看所有帖子标题，用户点击帖子标题可以对帖子内容进行查看，用户查看帖子内容之后，可以对帖子内容发布个人评论。

![](/images/0400ssm/ssm490/blog.019.png)

图5.7 在线论坛界面
### 5.2.3 更改个人信息
实现更改个人信息功能，其界面运行的效果图见图5.8。在该界面，用户及时检查并更改发生变化的个人简介信息，更改信息之后需要点击更新信息按钮才可以成功更改个人信息。

![](/images/0400ssm/ssm490/blog.020.png)
